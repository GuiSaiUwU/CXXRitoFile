...
namespace RitoFile {
    ...
    static constexpr float epsilon_subtracted = (1.0f - epsilon);
    
    static Quaternion slerp(const Quaternion& quat1, const Quaternion& quat2, float weight) {
        ...
        if (cos_omega > epsilon_subtracted) {
            ...
            if (std::fabs(sin_omega) < epsilon) {
                ...
                s1 = std::sin((1.0f - weight) * omega) * inv_sin_omega;
